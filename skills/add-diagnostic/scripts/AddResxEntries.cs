...
#:include Common.cs
// AddResxEntries.cs — adds diagnostic string resources to .resx files in ID order and validates the result.
//
// Usage:  dotnet AddResxEntries.cs -- --resx <file.resx> [--resx <file.ja.resx> ...]
//                                     --entries <json-array-or-path> [--ids-file <DiagnosticIds.cs>]
//                                     [--force] [--validate-only]
//
// Entries JSON: [{ "name": "FooShouldBarTitle", "value": "Foo should bar", "comment": "optional" }, ...]
//
...
if (cli.Get("ids-file") is { } idsFile && File.Exists(idsFile))
    foreach (var id in IdConst.Parse(File.ReadAllText(idsFile))) idMap[id.Name] = id.Value;
...
sealed record Entry(string Name, string Value, string? Comment);

sealed record SortKey(bool Known, string Primary, int Rank)
{
